...
- Пример создания Keyspace с помощью CQL (Cassandra Query Language):
  ```cql
  ...
  ```
- Keyspace группирует таблицы, связанные по логике приложения, и задаёт правила их хранения и репликации.
...
- Пример создания таблицы:
  ...
  ```
  Здесь строка с `user_id` может содержать только те столбцы, которые заполнены (например, `name` и `email`, но без `age`).
...
- **Пользовательские типы (UDT — User-Defined Types)**:
  ...
  - Пример:
    ...
    ```
    Здесь `address` — пользовательский тип, а `frozen` указывает, что тип неизменяемый (требуется для UDT в Cassandra).
...
- Пример:
  - Запрос: "Получить последние 10 постов пользователя по `user_id`".
  - Таблица:
    ```cql
    CREATE TABLE my_app.user_posts (
        user_id uuid,
    ...
    ```
    Здесь `user_id` — partition key, `post_id` — clustering key, а `CLUSTERING ORDER BY` сортирует посты по убыванию времени.
...
### Пример полного дизайна
Предположим, у нас есть приложение для блогов, где нужно:
1. Получить профиль пользователя по `user_id`.
2. Получить последние посты пользователя по `user_id`.

**Keyspace**:
```cql
CREATE KEYSPACE blog_app
WITH replication = {
  'class': 'SimpleStrategy',
  'replication_factor': 3
};
...
**Таблица для постов пользователя**:
```cql
CREATE TABLE blog_app.user_posts (
    user_id uuid,
    post_id timeuuid,
    content text,
    created_at timestamp,
...
**Запросы**:
- Получить профиль: `SELECT * FROM blog_app.users WHERE user_id = ?;`
- Получить последние 10 постов: `SELECT * FROM blog_app.user_posts WHERE user_id = ? LIMIT 10;`

---

### Преимущества подхода
- **Высокая производительность**: Запросы выполняются быстро, так как данные уже организованы под конкретные сценарии.
- **Масштабируемость**: Равномерное распределение данных по partition key позволяет легко добавлять узлы.
- **Гибкость**: Динамические столбцы и коллекции позволяют адаптировать модель данных под разные задачи.
...
### Преимущества и особенности
- **Высокая производительность записи**: CommitLog и Memtable обеспечивают быструю запись.
- **Отказоустойчивость**: Репликация, Hinted Handoff и Read Repair минимизируют риск потери данных.
- **Гибкость согласованности**: Настраиваемые уровни согласованности позволяют адаптировать Cassandra под разные сценарии.
- **Масштабируемость**: Партиционирование и виртуальные узлы упрощают добавление новых узлов.
...
- **Создание таблицы**:
  ...
  - Пример:
    ...
    ```
- **Изменение Keyspace**:
  - Можно изменить параметры репликации:
    ```cql
...
- **INSERT**:
  ...
  - Пример:
    ...
    VALUES (uuid(), 'Alice', 'alice@example.com', '2025-07-29T09:00:00Z');
    ```
- **UPDATE**:
  ...
  - Пример:
    ```cql
    UPDATE my_app.users
    SET email = 'alice.new@example.com', name = 'Alice Smith'
    WHERE user_id = <uuid>;
    ```
- **DELETE**:
  ...
  - Пример:
    ```cql
    DELETE FROM my_app.users WHERE user_id = <uuid>;
    DELETE email FROM my_app.users WHERE user_id = <uuid>;
    ```
- **SELECT**:
  - Извлекает данные из таблицы. Требует указания partition key в `WHERE` для эффективного выполнения.
  - Пример:
    ```cql
    SELECT name, email FROM my_app.users WHERE user_id = <uuid>;
    ```
  - Поддерживает сортировку (если есть clustering key) и ограничение количества строк:
    ```cql
    SELECT * FROM my_app.user_posts
    WHERE user_id = <uuid>
...
- **Коллекции**:
  - Cassandra поддерживает три типа коллекций: `List`, `Set` и `Map`.
  - Пример создания таблицы с коллекциями:
    ```cql
    CREATE TABLE my_app.users (
        user_id uuid,
        emails list<text>,
  ...
  - Операции с коллекциями:
    ...
    WHERE user_id = <uuid>;
    ```
  - Ограничения: Коллекции не должны быть слишком большими (рекомендуется до 64 КБ).
- **UDT (User-Defined Types)**:
  - Пользовательские типы позволяют создавать сложные структуры данных.
  - Пример:
    ```cql
    CREATE TYPE my_app.address (
        street text,
    ...
    INSERT INTO my_app.users (user_id, home_address)
    VALUES (uuid(), {street: '123 Main St', city: 'New York', zip_code: 10001});
    ```
  - Примечание: `frozen` указывает, что UDT неизменяемый (нельзя обновлять отдельные поля).
- **JSON**:
  - CQL поддерживает вставку и чтение данных в формате JSON для удобства.
  - Пример:
    ```cql
    INSERT INTO my_app.users JSON
    '{"user_id": "<uuid>", "name": "Alice", "email": "alice@example.com"}';

    SELECT JSON name, email FROM my_app.users WHERE user_id = <uuid>;
    ```
...
### Пример проектирования и использования CQL
Предположим, у нас есть приложение для блогов с запросами:
1. Получить профиль пользователя по `user_id`.
2. Получить посты пользователя за определённый период.
...
CREATE MATERIALIZED VIEW blog_app.posts_by_tag AS
    SELECT user_id, post_id, content, tags
    FROM blog_app.user_posts
    WHERE tags IS NOT NULL AND user_id IS NOT NULL AND post_id IS NOT NULL
    PRIMARY KEY (tags, user_id, post_id)
    WITH CLUSTERING ORDER BY (user_id, post_id);
```

**Запросы**:
- Профиль пользователя:
  ```cql
  SELECT name, email FROM blog_app.users WHERE user_id = <uuid>;
  ```
- Посты пользователя за период:
  ```cql
  SELECT content, created_at FROM blog_app.user_posts
  WHERE user_id = <uuid> AND post_id > minTimeuuid('2025-07-01T00:00:00Z')
  LIMIT 10;
  ```
- Посты по тегу:
  ```cql
  SELECT content FROM blog_app.posts_by_tag WHERE tags CONTAINS 'news';
  ```

---

### Преимущества и особенности
- **Простота CQL**: SQL-подобный синтаксис упрощает переход от реляционных баз.
- **Гибкость**: Коллекции, UDT и JSON позволяют моделировать сложные данные.
- **Ограничения для производительности**: Запросы должны быть оптимизированы под partition key, чтобы избежать сканирования кластера.
- **Масштабируемость**: CQL поддерживает распределённую природу Cassandra, но требует правильного дизайна данных.
