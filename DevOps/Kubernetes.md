...
Kubernetes — это open-source платформа для управления контейнерами. Она позволяет автоматизировать их развертывание и работу, используя
декларативный подход: вы описываете желаемое состояние системы, а Kubernetes сам его поддерживает. Платформа имеет большую экосистему и
широкую поддержку.

*   **Самовосстановление:** Автоматически перезапускает и заменяет сбойные контейнеры.
*   **Балансировку нагрузки:** Распределяет сетевой трафик для обеспечения стабильности.
*   **Автоматизацию развертываний и откатов:** Управляет обновлениями приложений, создавая и удаляя контейнеры согласно описанному состоянию.
*   **Управление хранилищами и конфигурациями:** Автоматически подключает хранилища и безопасно управляет секретными данными (паролями, ключами).
*   **Оптимизацию ресурсов:** Эффективно размещает контейнеры на серверах кластера, исходя из их потребностей в CPU и RAM.

#### Что Kubernetes НЕ делает:
*   **Это не готовая PaaS:** Он предоставляет основу, а не монолитное решение. Все компоненты (мониторинг, логирование, CI/CD) являются подключаемыми.
*   **Не собирает код:** Не заменяет CI/CD-системы.
*   **Не предоставляет встроенные сервисы:** Не включает базы данных, очереди сообщений или кеши. Их нужно разворачивать отдельно.
*   **Не имеет готового мониторинга:** Предоставляет только механизмы для интеграции с внешними системами логирования и оповещения.

---

### **Что такое OpenShift?**

OpenShift — это готовая к использованию платформа на основе Kubernetes, дополненная инструментами для безопасности, разработки (CI/CD) и
управления, что делает её решением "из коробки" для корпоративных задач.
...
Пространство имён (Namespace) можно рассматривать как виртуальный кластер внутри вашего физического кластера Kubernetes. 
Вы можете иметь несколько пространств имён, изолированных друг от друга, внутри одного кластера.
...
Egress — это исходящий сетевой трафик, когда данные покидают вашу сеть или устройство (например, отправка файла в интернет). В облаке за него часто платят, а для безопасности его контролируют через файрволы и DLP, чтобы предотвратить утечки данных.
