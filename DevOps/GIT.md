...
1.  **Репозиторий** — это папка проекта, которая хранит все файлы и полную историю их изменений.

2.  **Коммит (снимок)** — это сохранённое состояние всех файлов проекта в определённый момент. Каждый коммит имеет уникальный ID (хэш) и ссылку на предыдущий, создавая цепочку истории.

3.  **Внутреннее хранилище (.git)** — Git хранит данные в виде объектов:
    *   **Blob**: содержимое файла.
    *   **Tree**: структура папок (ссылки на blobs и другие trees).
    *   **Commit**: связывает Tree с метаданными (автор, сообщение) и предыдущим коммитом.
    *   **Tag**: метка для важного коммита (например, версия `v1.0`).

4.  **Три области работы**:
    *   **Рабочая область**: папка, где вы редактируете файлы.
    *   **Индекс (Staging Area)**: "черновик" для коммита, куда вы добавляете изменения командой `git add`.
    *   **Репозиторий**: место, где хранятся все коммиты.

5.  **Ветвление (Branches)** — ветка является подвижным указателем на коммит, что позволяет вести параллельную разработку. Слияние (merge) объединяет изменения из разных веток.

6.  **Эффективность хранения** — Git не копирует файлы целиком. Он переиспользует неизменённые данные и сохраняет только новые, что экономит место.

7.  **Удалённые репозитории** — это копии вашего репозитория на сервере (например, GitHub). Команды `git push` и `git pull` синхронизируют локальные и удалённые изменения.

### Основной процесс работы:
1.  **Редактируете** файлы в рабочей области.
2.  **Добавляете** нужные изменения в индекс (`git add`).
3.  **Сохраняете** снимок в репозиторий (`git commit`).
4.  **Отправляете** изменения на сервер (`git push`) или забираете их (`git pull`).
