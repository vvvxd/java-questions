...
**Domain-Driven Design (DDD)** — это методология разработки программного обеспечения для систем с высокой степенью бизнес-сложности.

Её основная цель — **прямое отражение бизнес-модели (домена) в коде**. Вместо того чтобы фокусироваться в первую очередь на технологиях (
базы данных, фреймворки), DDD ставит в центр бизнес-логику.

Ключевой элемент для достижения этой цели — создание **Единого Языка (Ubiquitous Language)**. Это общий, строгий словарь терминов,
разработанный совместно бизнес-экспертами и разработчиками. Этот язык используется во всех артефактах проекта: в устных обсуждениях, в
документации, в диаграммах и, что наиболее важно, в исходном коде (имена классов, методов, модулей).

**Проблема, которую решает DDD:** Разрыв в понимании между бизнес-требованиями и их технической реализацией, что в сложных системах приводит
к созданию ПО, не решающего реальные задачи бизнеса.

* **Domain (Домен):** Предметная область, для которой создается программное обеспечение. Например: "Система управления складом", "Платформа
  для онлайн-торговли".
* **Ubiquitous Language (Единый Язык):** Формализованный словарь терминов домена. Если бизнес-эксперт использует термин "Котировка", то в
  коде должен существовать класс `Quote`, а не `PriceOffer` или `Estimate`.
* **Bounded Context (Ограниченный Контекст):** Это основное понятие стратегического DDD. Представляет собой явную границу, внутри которой
  отдельная модель домена является целостной и последовательной. Внутри контекста каждый термин из Единого Языка имеет одно, четкое
  значение.

* **Пример из E-commerce:**
  * В контексте **"Каталога" (`Catalog Context`)** модель `Product` будет содержать атрибуты: `productId`, `name`, `description`, `images`.
  * В контексте **"Склада" (`Inventory Context`)** модель `Product` будет
  содержать `productId`, `stockKeepingUnit`, `quantityOnHand`, `location`.
  * В контексте **"Продаж" (`Sales Context`)** модель `Product` будет содержать `productId`, `price`, `taxRate`.

Использование Ограниченных Контекстов позволяет избежать создания одного гигантского, перегруженного класса `Product`, который сложно
поддерживать. Это позволяет каждому модулю системы быть сфокусированным и автономным.

* **Context Map (Карта Контекстов):** Диаграмма или документ, описывающий отношения и интеграционные паттерны между различными Ограниченными
  Контекстами. Она определяет, как контексты взаимодействуют: например, через API, общую базу данных или асинхронные события. Один из важных
  паттернов интеграции — **Anti-Corruption Layer (Антикоррупционный Слой)**, который является "переводчиком" между моделями двух разных
  контекстов, предотвращая проникновение деталей реализации одного контекста в другой.

Тактическое проектирование предоставляет набор конкретных паттернов для реализации доменной модели *внутри одного* Ограниченного Контекста.

**Основные строительные блоки:**

* **Entity (Сущность):** Объект, который обладает уникальным идентификатором и чья идентичность сохраняется на протяжении всего жизненного
  цикла, независимо от изменения его атрибутов. *Пример:* `Order` с идентификатором `orderId`. Даже если статус и содержимое заказа
  меняются, это остается тот же самый `Order`.

* **Value Object (Объект-Значение):** Неизменяемый (immutable) объект, который не имеет собственного идентификатора и определяется
  исключительно набором своих атрибутов. Два объекта-значения с одинаковыми атрибутами считаются эквивалентными и взаимозаменяемыми.
  *Пример:* `Money` (состоит из `amount` и `currency`), `DateRange` (состоит из `startDate` и `endDate`). При необходимости изменения
  создается новый экземпляр объекта-значения.

* **Aggregate (Агрегат):** Кластер из одной или нескольких связанных сущностей и объектов-значений, который рассматривается как единое целое
  для целей изменения данных. У агрегата есть одна точка входа — **Корень Агрегата (Aggregate Root)**.
* **Цель:** Обеспечение целостности данных и соблюдение **инвариантов** — бизнес-правил, которые должны быть истинными в любой момент
  времени для группы объектов.
* **Принцип работы:** Любые изменения объектов внутри агрегата могут производиться только через методы его корня. Внешние объекты могут
  ссылаться только на корень агрегата, но не на его внутренние части. Все изменения в рамках одной операции с агрегатом должны выполняться в
  одной транзакции.
* **Пример:** Агрегат `Order`, где `Order` — это корень, а `OrderItem` — внутренняя сущность. Инвариант: "Общая сумма заказа должна быть
  равна сумме цен всех его позиций". Чтобы добавить товар, вы вызываете метод `order.addItem(...)`. Этот метод внутри себя
  создает `OrderItem`, добавляет его в коллекцию и пересчитывает `totalAmount`, гарантируя соблюдение инварианта.

* **Repository (Репозиторий):** Абстракция, которая обеспечивает интерфейс для доступа к агрегатам, имитируя поведение коллекции в памяти.
  Он инкапсулирует логику получения и сохранения агрегатов в хранилище данных (например, в SQL или NoSQL базе данных), изолируя доменную
  модель от инфраструктурных деталей. *Пример:* `IOrderRepository` с методами `findById(orderId)` и `save(Order aggregate)`.

* **Service (Сервис Домена):** Объект, реализующий операцию, которая не является естественной обязанностью какой-либо сущности или
  объекта-значения. Сервисы часто координируют действия между несколькими агрегатами. *Пример:* `MoneyTransferService`, который выполняет
  операцию перевода средств, затрагивающую два разных агрегата `Account`.

* **Domain Event (Событие Домена):** Объект, который представляет собой факт некоторого значимого события, произошедшего в домене в прошлом.
  События используются для организации слабой связности между разными частями системы (часто между разными Ограниченными Контекстами).
  *Пример:* `OrderPlaced`, `CustomerRegistered`. Когда агрегат `Order` успешно создан, он может опубликовать событие `OrderPlaced`. Другие
  части системы могут подписаться на это событие и отреагировать соответствующим образом (например, модуль уведомлений отправит email).
