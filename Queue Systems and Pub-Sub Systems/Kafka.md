...
* **Log Compaction (Сжатие лога):** Специальный режим удержания. Вместо удаления по времени/размеру Kafka гарантирует сохранение как минимум
  последней записи для каждого уникального ключа. Старые записи с тем же ключом удаляются в процессе фоновой очистки. Это позволяет
  использовать топик как базу данных для хранения состояний (changelog), где важна только последняя версия для каждого ключа.

### Внутреннее устройство и архитектура (Сердце Kafka)

* **Репликация (Replication):**
    * **Leader и Follower:** Понимать, что запись и чтение всегда идут через лидера партиции. Фолловеры просто копируют данные.
    * **In-Sync Replicas (ISR):** Критически важный концепт. Это множество реплик (включая лидера), которые не сильно отстали от лидера.
      Понимать, как ISR влияет на долговечность данных.
    * **`acks` (Acknowledgements):** Связь между продюсером и репликацией.
        * `acks=0`: Отправил и забыл (максимальная производительность, риск потери).
        * `acks=1`: Ждем подтверждения только от лидера (баланс, но есть риск потери при падении лидера до репликации).
        * `acks=all` (или `-1`): Ждем подтверждения от всех реплик в ISR (максимальная гарантия, самая низкая производительность).
    * **`min.insync.replicas`:** Настройка брокера, которая не даст продюсеру с `acks=all` записать данные, если в ISR меньше указанного
      числа реплик. Это гарантия отказоустойчивости.

* **Хранение данных на диске:**
    * **Log Segments:** Партиции на диске — это не один большой файл, а набор файлов-сегментов. Понимать, как это позволяет эффективно
      управлять данными и применять retention-политики.
    * **Индексы:** Для каждой партиции есть индекс (offset index, time index), который позволяет Kafka очень быстро находить нужное
      сообщение по смещению (offset) или времени, не сканируя весь лог.
    * **Zero-Copy:** Ключевая оптимизация производительности. Понимать, как Kafka отдает данные консьюмерам напрямую из page cache
      операционной системы, минуя копирование в пространство приложения.
    * **Log Compaction:** Альтернатива удалению по времени/размеру. Понимать ее назначение: хранить только последнее значение для каждого
      ключа. Идеально для потоков изменений (change data capture) или хранения состояний.

* **Роль ZooKeeper и переход на KRaft:**
    * **ZooKeeper (в старых версиях):** Понимать, что он хранил метаданные кластера: список брокеров, конфигурации топиков, ACL, кто
      является лидером для какой партиции. Знать, что это было узким местом.
    * **KRaft (Kafka Raft):** Понимать, что Kafka теперь может работать без ZooKeeper, храня метаданные в специальном внутреннем топике,
      используя консенсус-протокол Raft. Это упрощает архитектуру и повышает производительность.

* **Controller Broker:** "Мозг" кластера. Один из брокеров выбирается контроллером. Его задачи: управление лидерством партиций, уведомление
  других брокеров об изменениях и т.д.
