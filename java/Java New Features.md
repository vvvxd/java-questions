...
**Описание**: Виртуальные потоки (JEP 444, стандартизированы в Java 21 после preview в Java 19–20) — это легковесные потоки, управляемые JVM, а не операционной системой, что позволяет создавать миллионы потоков для высококонкурентных приложений.
...
**Внутреннее устройство**:
- Виртуальные потоки — это объекты `java.lang.Thread`, но они не привязаны напрямую к системным потокам (carrier threads).
- JVM использует **continuations** (механизм сохранения и восстановления состояния стека) для управления выполнением виртуальных потоков.
- Множество виртуальных потоков маппится на небольшое количество системных потоков через `ForkJoinPool` (по умолчанию `commonPool`).
- Блокирующие операции (например, `Thread.sleep`, I/O через `java.nio`) автоматически освобождают системный поток (yield), позволяя JVM переключиться на другой виртуальный поток. Это реализовано через **parking** и **unparking** с использованием `Continuation` API.
- JVM оптимизирует переключение контекста, минимизируя накладные расходы по сравнению с системными потоками.

**Особенности**:
- Виртуальные потоки идеальны для I/O-интенсивных приложений (например, веб-серверов), но не для CPU-интенсивных задач, где системные потоки предпочтительнее.
- Поддерживают стандартные API `Thread` (например, `start()`, `join()`).
- Совместимы с существующими инструментами JVM (JFR, JMX) для профилирования.

**Оптимизации**:
- Низкие накладные расходы на создание и переключение потоков (порядка микросекунд).
- JIT-компилятор оптимизирует вызовы `Continuation.yield` для переключения контекста.
- Пул системных потоков адаптируется к нагрузке, минимизируя конкуренцию за ресурсы.

**Преимущества**:
- Поддержка миллионов потоков без значительного потребления памяти.
- Упрощение асинхронного программирования по сравнению с `CompletableFuture` или реактивными фреймворками.

**Ограничения**:
- Рефлексия и нативный код могут нарушать работу виртуальных потоков.
- Требуется осторожность с синхронизацией, так как `synchronized` блоки могут удерживать системный поток.
