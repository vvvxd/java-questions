...
1. [Дайте краткую характеристику Enum в Java](#1-дайте-краткую-характеристику-enum-в-java)
2. [Внутренняя структура java.lang.Enum](#2-внутренняя-структура-javalangenum)
3. [Как происходит создание и хранение экземпляров Enum](#3-как-происходит-создание-и-хранение-экземпляров-enum)
4. [Ключевые особенности работы Enum](#4-ключевые-особенности-работы-enum)
5. [Какие расширенные возможности Enum](#5-какие-расширенные-возможности-enum)
6. [Как Enum работает в JVM](#6-как-enum-работает-в-jvm)
7. [Производительность и память Enum](#7-производительность-и-память-enum)
8. [Может Enum реализовывать (implements) интерфейсы?](#8-может-enum-реализовывать-implements-интерфейсы)
9. [Может Enum расширять (extends) класс?](#9-может-enum-расширять-extends-класс)
10. [Можно ли создать Enum без экземпляров объектов?](#10-можно-ли-создать-enum-без-экземпляров-объектов)
11. [Можно ли переопределить метод toString() для Enum?](#11-можно-ли-переопределить-метод-tostring-для-enum)
12. [Что будет, если не переопределять метод toString() для Enum?](#12-что-будет-если-не-переопределять-метод-tostring-для-enum)
13. [Можно ли указывать конструктор внутри Enum?](#13-можно-ли-указывать-конструктор-внутри-enum)
14. [Что делает метод ordinal() в Enum?](#14-что-делает-метод-ordinal-в-enum)
15. [Можно ли использовать Enum с TreeSet или TreeMap в Java?](#15-можно-ли-использовать-enum-с-treeset-или-treemap-в-java)
16. [Как связаны методы ordinal() и compareTo() в Enum?](#16-как-связаны-методы-ordinal-и-compareto-в-enum)
17. [Можно ли использовать Enum в switch case?](#17-можно-ли-использовать-enum-в-switch-case)
18. [Как получить все имеющиеся значения в экземпляре Enum?](#18-как-получить-все-имеющиеся-значения-в-экземпляре-enum)
