...
1. [Что такое I/O в Java и какие основные категории операций ввода-вывода существуют?](#1-что-такое-io-в-java-и-какие-основные-категории-операций-ввода-вывода-существуют)
2. [Как устроены байтовые потоки в Java (`InputStream` и `OutputStream`)?](#2-как-устроены-байтовые-потоки-в-java-inputstream-и-outputstream)
3. [Как работают символьные потоки (`Reader` и `Writer`)?](#3-как-работают-символьные-потоки-reader-и-writer)
4. [Что такое буферизация и как она реализована в Java I/O?](#4-что-такое-буферизация-и-как-она-реализована-в-java-io)
5. [Что такое `java.nio` и как оно отличается от `java.io`?](#5-что-такое-javanio-и-как-оно-отличается-от-javaio)
6. [Как работает `Buffer` в NIO и каковы его основные характеристики?](#6-как-работает-buffer-в-nio-и-каковы-его-основные-характеристики)
7. [Что такое `Channel` и как он взаимодействует с буферами?](#7-что-такое-channel-и-как-он-взаимодействует-с-буферами)
8. [Как работает неблокирующий ввод/вывод с использованием `Selector`?](#8-как-работает-неблокирующий-вводвывод-с-использованием-selector)
9. [Как кодировки обрабатываются в NIO?](#9-как-кодировки-обрабатываются-в-nio)
10. [Как NIO взаимодействует с операционной системой?](#10-как-nio-взаимодействует-с-операционной-системой)
11. [Как работает `java.nio.file` (NIO.2) и какие возможности он предоставляет?](#11-как-работает-javaniofile-nio2-и-какие-возможности-он-предоставляет)
12. [Как обрабатываются ошибки в Java I/O?](#12-как-обрабатываются-ошибки-в-java-io)
13. [Как работает сериализация в Java и как она связана с I/O?](#13-как-работает-сериализация-в-java-и-как-она-связана-с-io)
14. [Как Java I/O взаимодействует с операционной системой?](#14-как-java-io-взаимодействует-с-операционной-системой)
15. [Как оптимизировать производительность I/O в Java?](#15-как-оптимизировать-производительность-io-в-java)
16. [Какие ограничения и проблемы есть в Java I/O?](#16-какие-ограничения-и-проблемы-есть-в-java-io)
...
Если у вас есть дополнительные вопросы или вы хотите углубиться в конкретный аспект (например, асинхронный I/O или сериализация), дайте знать!